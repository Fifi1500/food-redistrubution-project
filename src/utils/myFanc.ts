...
export const getWilayaName = (code: string): string => {
  return WILAYAS[code as keyof typeof WILAYAS] || code;
};

export const getWilayaCode = (name: string): string | null => {
  const entry = Object.entries(WILAYAS).find(([_, value]) => value === name);
  return entry ? entry[0] : null;
};

export const getAllWilayas = (): { code: string; name: string }[] => {
  return Object.entries(WILAYAS).map(([code, name]) => ({ code, name }));
};

export const isValidWilaya = (code: string): boolean => {
  return Object.keys(WILAYAS).includes(code);
};
